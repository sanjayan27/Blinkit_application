...
export async function userLoginController(req, res) {
  try {
    ...
    const updateUserDate = await UserModel.findByIdAndUpdate(checkEmail?._id, {
...
export async function forgetPasswordController(req, res) {
  try {
    ...
    const userValid = await UserModel.findOne({ email: normalizedEmail });
...
export async function verifyEmailOtpController(req, res) {
  try {
    ...
    const removeOtp = await UserModel.findByIdAndUpdate(checkEmail._id, {
      forgot_password_otp: "",
...
export async function passwordResetController(req, res) {
  try {
    ...
    const hashPassword = await bcryptjs.hash(password, salt);
    //storing db
    const storeNewPass = await UserModel.updateOne(
      { email: email },
      {
        password: hashPassword,
      }
...
export async function userDetailsController(req, res) {
  try {
    ...
    const checkUserinDb = await UserModel.findOne({ _id: userId }).select(
      "-password -refesh_token  "
    );
    if (!checkUserinDb) {
      return res.status(400).json({
        message: "user not found",
